...
myoscillator = GoodwinOscillator()
...
for p in range(len(parameters)):
    ...
    data = pd.read_csv("./results/bifurcations/goodwin_{}_allinfo.dat".format(
    ...
    label = '$k_2$' if (p== 0) else ('$k_4$' if (p==1) else \
           '$k_6$')
    x_limits = [24,27]
    y_limits = [1,2]
    ax3.set_xlabel('period (h)') 
    ax3.plot(periods, (max_x-min_x)/mean, 'o', label=label,
            c=colors[p], markersize=4, alpha=0.6)
...
if not isExist:  
    os.makedirs('./figures/')
#g.fig.savefig('./figures/fig3a.pdf', format='pdf')    